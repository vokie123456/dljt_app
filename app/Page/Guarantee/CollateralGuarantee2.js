...
const DatePicker1 = (props) => {
    const {datePickerDisabled} =  props.props.data.props;
    return (
        ...
        <ScrollView>
            <KeyboardAwareScrollView>
                <View style={{paddingLeft: px2dp(36), backgroundColor: '#fff', marginTop: px2dp(20)}}>
                    ...
                    <DefaultSelect require placeholder={'请选择'} name={'所有人类型 : '} disable
                    ...
                    <DefaultInput require placeholder={'请输入'} name={'协议价值（元） : '}
                                  ...
                                  props={props}/>
                    <DefaultInput placeholder={'请输入'} name={'抵押价值（元）: '}
                                  onChangeText={props.onChangeValuationMechanism} valOne={(isEmpty(props.valuationMechanism) ? props.valuationMechanism : props.valuationMechanism + "")}
                                  props={props}/>
                    <DefaultInput   placeholder={'请输入'} name={'抵质押率（%）: '}
                                  onChangeText={props.onChangeAssuremoneye} valOne={(isEmpty(props.assuremoney) ? props.assuremoney : props.assuremoney + "")}
                                  props={props}/>
                    <DatePicker1 props={props} name={"评估时间 : "} onChangeDate={props.onSelectedDateStatus}/>
                    <DefaultInput placeholder={'请输入'} name={'备        注 : '}
                                  onChangeText={props.onSelectedMortgageRemarks} valOne={(isEmpty(props.mortgageRemarks) ? props.mortgageRemarks : props.mortgageRemarks + "")}
                                  props={props}/>
                </View>
...
export default class CollateralGuarantee2 extends Component {
    // 构造
    constructor(props) {
        super(props);
        // 初始状态
        ...
        if (props.type1 === 'see') {
            this.state = {
                ...
                mortgageRemarks: '',//备注
                finalprice: '',//原值
                finalCertificationPrice: null,//协议价值
                assuremoney: '0',//抵质押率
                valuationMechanism: '',//抵押价值
    ...
    //查看信息
    _seeInfo() {
        let url = Config.baseApi + Config.twoApi.seeVehicle + this.state.id+"&businessType="+this.props.data.businessType;//获取抵押详情
    ...
    _render = (list) => {
        const {assuretypeid,typeid,personTypeId,relation,assureofname,mortgagepersontypeforvalue, assureofnameEnterOrPerson, valuationMechanism, valuationTime, assuremoney, mortgageRemarks, finalprice, finalCertificationPrice}=list.vProcreditDictionary;
        this.setState({
            visible: false,
        })
                ...
                <FormInput
                    ...
                    onChangeFinalCertificationPrice={(text) => {
                        this.setState({
                            ...
                            assuremoney:(valuationMechanism/text)
                        })
                    }}
                    ...
                    onChangeValuationMechanism={(text) => {
                        this.setState({
                            ...
                            assuremoney:(text/finalCertificationPrice)
                        })
                    }}
