...
export default class GuaranteeMaterial extends Component {
    ...
    componentDidMount() {
        this.setState({});
        this._seeInfo();//查询担保材料清单
        // this._seeInfo1();//查询担保材料清单
    }

    componentWillReceiveProps(nextProps) {
        if (nextProps.isRefresh) {
            this._seeInfo();//回调担保材料清单
        }
    }
    ...
    _seeInfo() {
        ...
        let url = Config.baseApi + Config.twoApi.listEnterprise + this.props.data.data.vars[0].projectId+"&businessType="+this.props.data.data.vars[0].businessType;
        RTRequest.fetch1(url).then((responseText) => {
            if (responseText) {
                console.log('担保材料清单', responseText);
    ...
    _renderRow(rowData, sectionID, rowID) {
        ...
        const swipeOutBtn = [
            ...
            },
            {
                ...
                type: 'primary',
                onPress: () => {
                    Actions.GuaranteeMaterial2({data: rowData.item.value, inputDisabled: this1.props.inputDisabled});
                },
            },
        ];
        const swipeOutBtn1 = [
