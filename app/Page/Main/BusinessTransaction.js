        ...
        <View style={{
          borderBottomWidth: props.border ? 0 : StyleSheet.hairlineWidth, paddingTop: px2dp(30)
            ...
            <View style={{
                ...
                {/*<Text style={{fontSize: px2dp(23), color: 'green', marginLeft: px2dp(-200)}}>({props.runStatus})</Text>*/}
                <Text style={{fontSize: px2dp(23), color: 'red'}}>{props.projectMoney+'元'}</Text>
            </View>
            <View style={{
                ...
                <Text style={{fontSize: px2dp(23), color: '#333'}}>{isEmpty(props.duedate)?props.duedate:''}</Text>
            </View>
        </View>
    )
}
export default class BusinessTransaction extends Component {
    ...
    constructor(props) {
        super(props);
        console.log("上个页面参数-----BusinessTransaction", props);
        // 初始状态
        this.state = {
            prodData: [],
            id: props.id,
    ...
    componentWillReceiveProps( nextProps){
        console.log("上个页面参数-----BusinessTransaction", nextProps);
        this.setState({
            id: nextProps.id,
     ...
     _getByIdPerson = (id,judge) => {
        let url = Config.baseApi + Config.processApi.getAllFlowById + id+'&type='+judge;
        console.log("-----------------------------", url);
    ...
    _renderRow(rowData) {
        ...
        return (
            <View>
                <DefaultInput name={Tool.isBusinessType(businessType)} uname={this1._subStr1(subject)+projectNumber} duedate={this1._subStr2(duedate+'')} projectMoney={isEmpty(projectMoney)?0:projectMoney}/>
            </View>
        );
    }
