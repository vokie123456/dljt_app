...
export default class CommissionList extends Component {
    ...
    _seeInfo() {
        let url = Config.baseApi + Config.processApi.listByProject  + this.props.data.data.vars[0].projectId;
    ...
    _render = (list) => {
        ...
        for (let i = 0; i < dataBlob.length; i++) {
            sumMoney += dataBlob[i].value.incomeMoney
        }
        this.setState({
            prodData:dataBlob,
            sumMoney:sumMoney,
    ...
    _deleteItem(rowID) {
        let url = Config.baseApi + Config.processApi.deleteSlActualToCharge  + rowID;
        RTRequest.fetch1(url).then((responseText) => {
            if (responseText) {
                console.log('删除手续费用清单', responseText);
    ...
    _renderRow(rowData, sectionID, rowID) {
        ...
        const this1 = this;
        let {intentDate,incomeMoney,planChargeId,actualChargeId,typeName} = rowData.item.value;
        const swipeOutBtn = [
            {
                text: '删除',
                type: 'delete',
				...
				>
					<View style={styles.contacts}>
						<TouchableOpacity
                            ...
                            activeOpacity={.8}
                            onPress={ ()=>Actions.AddCommissionList({data:rowData.item.value,allData:this1.props.data,type1: "see",data1:this1.props})}>
							<View style={styles.leftText}>
								<View style={{paddingLeft: px2dp(15)}}>
									<Text style={{color: 'black',marginBottom: px2dp(1),fontSize:px2dp(28)}}>{typeName}</Text>
									<Text>{intentDate}</Text>
								</View>
							</View>
							<View style={styles.leftText1}>
								<Text style={{fontSize: px2dp(29),color: '#ff4800',paddingRight: px2dp(15)}}>{incomeMoney}</Text>
