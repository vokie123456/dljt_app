...
export default class BaoZMortgageViewList extends Component {
    ...
    _seeInfo() {
        let url = Config.baseApi + Config.processApi.findBZList  + this.props.data.data.vars[0].projectId;
    ...
    _goDetail = (value,data)=> {
        if(value.personTypeId == 602){
            Actions.CustomerMortgageDetail({data: data,personstype:value});
        }else if(value.personTypeId == 603){
            Actions.PersonMortgageDetail({data: data,personstype:value})
        }
    }
    _renderRow(rowData, sectionID, rowID) {
        const this1 = this;
        console.log('删除还款来源', rowData.item.value);
        let {assuremodeidValue,personTypeValue,assureofnameEnterOrPerson,} = rowData.item.value;
        const swipeOutBtn = [
            {
                text: '删除',
                type: 'delete',
                ...
                >
                    <View style={styles.contacts}>
                        <TouchableOpacity
                            ...
                            activeOpacity={.8}
                            onPress={()=>this._goDetail(rowData.item.value,this.props.data)}
                        >
                            <View
    ...
    render() {
        return (
            <View style={styles.container}>
                <Title name={this.props.title} rightText={this.props.inputDisabled?"":"md-add"} back onPress="BaoZMortgageViewDetail" type="add" {...this.props.data}/>
                <ScrollView
