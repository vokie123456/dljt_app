        ...
        <ScrollView style={{}}>
            <KeyboardAwareScrollView>
                <View style={{backgroundColor: '#fff',}}>
                    ...
                    <DefaultInput disabled={true} require placeholder={'请输入企业名称'} props={props} value={props.e_value} name={'企业名称：'}
                                  onChangeText={props.onChangeEnterpriseNameText}/>
                    <DefaultInput placeholder={'请输入企业简称'} value={props.s_value} props={props} name={'企业简称：'}
                                  onChangeText={props.onChangeShortNameText}/>
                    <DefaultSelect require placeholder={'请选择'} name={'证件类型：'} props={props} value={props.maritalStatusValue}
                                   items={props.maritalStatusItems} onSelected={props.onSelectedmaritalStatus}/>
                    {props.documentType==1? <DefaultInput require placeholder={'请输入社会信用代码'} props={props} value={props.c_value} name={'社会信用代码：'} onChangeText={props.onChangeSocialCreditText}/>:null}
                    {props.documentType==2? <DefaultInput require placeholder={'请输入税务登记号码'} props={props} value={props.t_value} name={'税务登记号码：'} onChangeText={props.onChangeTaxationRegisterText}/>:null}
                    {props.documentType==2?<DefaultInput require placeholder={'请输入营业执照号码'} props={props} value={props. cc_value} name={'营业执照号码：'} onChangeText={props.onChangeBusinessLicenseText}/>:null}
                    {props.documentType==2?<DefaultInput require placeholder={'请输入组织机构代码'} props={props} value={props.o_value} name={'组织机构代码：'} onChangeText={props.onChangeOrganizationText}/>:null}
                    <DefaultInput require placeholder={'请输入联系电话'} props={props} value={props.tele_value} name={'联系电话：'} onChangeText={props.onChangeConcatPhoneText}/>
                    <DefaultInput require placeholder={'请输入通讯地址'} props={props} value={props.area_value} name={'通讯地址：'} onChangeText={props.onChangeCommunicateAreaText}/>
                    <DefaultInput  placeholder={'请输入邮政编码'} props={props} value={props.postCode} name={'邮政编码：'} onChangeText={props.onChangePostCodeText}/>
...
export default class EnterpriseCusBasicInfo extends Component {
    ...
    constructor(props) {
        super(props);
        //初始化状态
        console.log(11111, props.data.data, props.data.allData);
        this.state = {
            visible: true,
            legalpersonid: "",
            shopId: "",
            ...
            RTRequest.fetch1(url).then(responseText => {
                if (responseText) {
                    ...
                    if (!isEmpty(responseText.data)) {
                        ...
                        const {shopId,postcoding, shopName, enterprisename, shortname, documentType, taxnum, cciaa, organizecode, linkman, telephone, area, hangyeType, fixedAssetsInvestment, totalLoan}=responseText.data.enterprise;
                        this.setState({
                            visible: false,
    ...
    render() {
        ...
        const {data1,postCode,legalPersonName, cardNum, email, cellPhone, sexValue, cardTypeValue, enterpriseName, shortName, creditAccountNum, documentType, taxNum, cciaa, organizeCode, linkMan, telePhone, area, hangyeType, fixedAssetsInvestment, totalLoan}=this.state;
        console.log('-------------------',data1);
        return (
            <View style={{flex: 1}}>
                ...
                <View style={{
                    ...
                    {data1.selectDisabled?<View/>:<SmallButton style={{flex: 1}} name="保存" height={70} width={240}
                                 onPress={() => this._saveEnterpriseBasicInfo()}/>}
                </View>
            </View>
        )
    }
    ...
    _saveEnterpriseBasicInfo=()=> {
        const {postCode,businessType,productId,operationType,oppositeType,creater,legalPersonName,sexValue,cardNum,cardTypeValue,email,cellPhone,legalpersonid,shopId,shopName,id,enterpriseName,shortName,creditAccountNum,documentType,taxNum,cciaa,organizeCode,linkMan,telePhone,area,hangyeType,fixedAssetsInvestment,totalLoan,createrId,createdate,companyId,salesArea,ownership,opendate,employeetotal,jyplace,areaMeasure,
